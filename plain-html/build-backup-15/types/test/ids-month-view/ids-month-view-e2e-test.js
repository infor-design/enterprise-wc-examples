...
describe('Ids Month View e2e Tests', () => {
    ...
    it('should display correct number of days in a month', async () => {
        ...
        let numberOfDays = await page.$eval(name, (el) => el.shadowRoot.querySelectorAll('td:not(.alternate)').length);
        ...
        numberOfDays = await page.$eval(name, (el) => el.shadowRoot.querySelectorAll('td:not(.alternate)').length);
        ...
        await page.$eval(name, (el) => el.changeDate('today'));
        day = await page.$eval(name, (el) => el.shadowRoot.querySelector('td.is-selected').dataset.day);
        month = await page.$eval(name, (el) => el.shadowRoot.querySelector('td.is-selected').dataset.month);
        year = await page.$eval(name, (el) => el.shadowRoot.querySelector('td.is-selected').dataset.year);
        dayText = await page.$eval(name, (el) => el.shadowRoot.querySelector('td.is-selected').textContent);
    ...
    it('should be selectable', async () => {
        ...
        await page.$eval(name, (el) => el.changeDate('next-month'));
        ...
        await page.$eval(name, (el) => el.changeDate('previous-month'));
        year = await page.$eval(name, (el) => el.year);
        expect(year).toEqual(2021);
    ...
    it('should support changing locale', async () => {
        ...
        await page.waitForFunction(() => document.querySelector('ids-month-view')?.locale?.calendar().name === 'islamic-umalqura');
        // RTL check
        ...
        expect(numberOfDays).toEqual(29);
        // Selectable
        await page.$eval(name, (el) => el.shadowRoot.querySelector('td:not(.alternate)')?.click());
        numberOfDays = await page.$eval(name, (el) => el.shadowRoot.querySelector('td.is-selected')?.textContent);
        ...
        // Wait till calendars load
        await page.waitForFunction(() => document.querySelector('ids-month-view')?.locale?.calendar().name === 'gregorian');
        // RTL check
    ...
    it('should trigger compact view and today button', async () => {
        let hasFullSizeClass = await page.$eval(name, (el) => el.container.classList.contains('is-fullsize'));
        ...
        hasFullSizeClass = await page.$eval(name, (el) => el.container.classList.contains('is-fullsize'));
        const hasCompactClass = await page.$eval(name, (el) => el.container.classList.contains('is-compact'));
        ...
        const hasTodayBtn = await page.$eval(name, (el) => el.shadowRoot.querySelector('.btn-today'));
    ...
    it('should handle legend items', async () => {
        ...
        const weekendByDay = await page.$eval(name, (el) => {
            ...
            return Array.from(days)?.every((item) => item.classList.contains('has-legend')
    ...
    it('should handle range selection', async () => {
        ...
        await page.$eval(name, (el) => el?.container.querySelector('td[data-month="10"][data-year="2021"][data-day="1"]')?.click());
        let start = await page.$eval(name, (el) => el?.rangeSettings.start);
        expect(start).not.toBeNull();
        await page.$eval(name, (el) => el?.container.querySelector('td[data-month="10"][data-year="2021"][data-day="4"]')?.click());
        ...
        await page.$eval(name, (el) => el?.container.querySelector('td[data-month="10"][data-year="2021"][data-day="16"]')?.click());
        ...
        expect(end).not.toBeNull();
        expect(selected).toEqual(8);
        await page.$eval(name, (el) => el?.container.querySelector('td[data-month="10"][data-year="2021"][data-day="2"]')?.click());
        await page.keyboard.press('ArrowLeft');
        await page.keyboard.press('ArrowLeft');
        let rangeDay = await page.$eval(name, (el) => el?.container.querySelector('.range-prev')?.dataset.day);
        expect(+rangeDay).toEqual(1);
        await page.keyboard.press('Escape');
        start = await page.$eval(name, (el) => el?.rangeSettings.start);
        expect(start).toBeNull();
        await page.$eval(name, (el) => el?.container.querySelector('td[data-month="10"][data-year="2021"][data-day="29"]')?.click());
        await page.keyboard.press('ArrowRight');
        await page.keyboard.press('ArrowRight');
        rangeDay = await page.$eval(name, (el) => el?.container.querySelector('.range-next')?.dataset.day);
    ...
    it('should show disabled dates visually', async () => {
        ...
        // Weekends
        let isDisabled = await page.$eval(name, (el) => {
            const firstDays = el?.container.querySelectorAll('.month-view-table tr td:first-child');
            const lastDays = el?.container.querySelectorAll('.month-view-table tr td:last-child');
            return [...firstDays, ...lastDays].every((item) => item.classList.contains('is-disabled'));
        });
        ...
        isDisabled = await page.$eval(name, (el) => {
            const firstDays = el?.container.querySelectorAll('.month-view-table tr td:first-child');
            const lastDays = el?.container.querySelectorAll('.month-view-table tr td:last-child');
            return [...firstDays, ...lastDays].every((item) => item.classList.contains('is-disabled'));
        });
        ...
        // Dates
        isDisabled = await page.$eval(name, (el) => {
            const disabledDate = el?.container.querySelector('td[data-year="2021"][data-month="10"][data-day="16"]');
        ...
        isDisabled = await page.$eval(name, (el) => {
            const minDateRow = el?.container.querySelectorAll('.month-view-table tr:first-child td');
            const maxDateRow = el?.container.querySelectorAll('.month-view-table tr:last-child td');
            return [...minDateRow, ...maxDateRow].every((item) => item.classList.contains('is-disabled'));
        });
        expect(isDisabled).toBeTruthy();
        await page.evaluate((el) => {
            const element = document.querySelector(el);
        ...
        expect(isDisabled).toBeFalsy();
        isDisabled = await page.$eval(name, (el) => {
            const rows = el?.container.querySelectorAll(`.month-view-table tr:nth-child(2) td,
        .month-view-table tr:nth-child(3) td,
        .month-view-table tr:nth-child(4) td`);
            return [...rows].every((item) => item.classList.contains('is-disabled'));
        });
        expect(isDisabled).toBeTruthy();
        // Years
        await page.evaluate((el) => {
        ...
        isDisabled = await page.$eval(name, (el) => {
            const month = el?.container.querySelectorAll('.month-view-table tr td');
            return [...month].every((item) => item.classList.contains('is-disabled'));
        });
        expect(isDisabled).toBeTruthy();
    });
    it('should show today visually', async () => {
        ...
        const todayEl = await page.$eval(name, (el) => el?.container.querySelector('td.is-today'));
        expect(todayEl).not.toBeNull();
    });
    it.skip('should show week numbers picklist', async () => {
        await page.evaluate(() => {
            document.querySelector('ids-container')?.insertAdjacentHTML('afterbegin', `<ids-month-view
        ...
        ></ids-month-view>`);
            // Expand picklist
            const datePicker = document.querySelector('#e2e-month-view-week')?.container.querySelector('ids-date-picker');
            if (datePicker) {
                datePicker.expanded = true;
            }
        });
        const getSelectedWeek = () => page.$eval('#e2e-month-view-week', (el) => el?.container.querySelector('ids-date-picker')
            ?.container.querySelector('.is-week.is-selected')?.textContent);
        // If week numbers picklist is set month picklist doesn't appear
        const monthPicker = await page.$eval('#e2e-month-view-week', (el) => el?.container.querySelector('.is-month'));
        expect(monthPicker).toBeNull();
        // Current week number
        expect(+await getSelectedWeek()).toEqual(23);
        // Select another week, first in the list
        await page.$eval('#e2e-month-view-week', (el) => el?.container.querySelector('ids-date-picker')?.container.querySelector('.is-week')?.click());
        expect(+await getSelectedWeek()).toEqual(21);
        // Up/down buttons
        await page.$eval('#e2e-month-view-week', (el) => el?.container.querySelector('ids-date-picker')?.container.querySelector('.is-week-nav')?.click());
        expect(+await getSelectedWeek()).toEqual(15);
        await page.$eval('#e2e-month-view-week', (el) => el?.container.querySelector('ids-date-picker')
            ?.container.querySelector('.is-btn-down.is-week-nav')?.click());
        expect(+await getSelectedWeek()).toEqual(21);
    });
});
//# sourceMappingURL=ids-month-view-e2e-test.js.map