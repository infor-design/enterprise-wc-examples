...
const dataGrid = document.querySelector('#data-grid-paging-server-side');
(async function init() {
    const columns = [];
    // Set up columns
    columns.push({
    ...
    const populatePaginatedDatagrid = async (pageNumber = 1, pageSize = 10) => {
        const { results, totalResults, } = await mockServerSideRequest(pageNumber, pageSize);
...
//# sourceMappingURL=pagination-server-side.js.map