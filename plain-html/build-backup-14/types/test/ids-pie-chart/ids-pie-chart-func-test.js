...
describe('IdsPieChart Component', () => {
    ...
    it('shows a tooltip on hover', async () => {
        ...
        const tooltip = pieChart.shadowRoot.querySelector('ids-tooltip');
        await processAnimFrame();
    ...
    it('shows tooltip on hover with donut', () => {
        ...
        pieChart.data = ds;
        const tooltip = pieChart.shadowRoot.querySelector('ids-tooltip');
        [...pieChart.container.querySelectorAll('.slice')].forEach((slice, i) => {
            slice.dispatchEvent(new CustomEvent('hoverend'));
            expect(tooltip.visible).toEqual(true);
    ...
    it('can set a legend formatter', async () => {
        pieChart.legendFormatter = (slice, datax) => `${slice.name}: ${datax.rounded}`;
    ...
    it('should select/deselect by click', () => {
        ...
        pieChart.selectable = true;
        let selected = pieChart.selectionElements.filter((el) => el.hasAttribute('selected'));
        expect(selected.length).toEqual(0);
        let elem = pieChart.shadowRoot.querySelector('.chart-legend-item:nth-child(1)');
        triggerClick(elem);
        selected = pieChart.selectionElements.filter((el) => el.hasAttribute('selected'));
    ...
    it('should set pre selected elements', async () => {
        ...
        pieChart.data = ds;
        await processAnimFrame();
        const selected = pieChart.selectionElements.filter((el) => el.hasAttribute('selected'));
        expect(selected.length).toEqual(1);
    });
    it('should veto before selected', async () => {
        let veto;
        pieChart.addEventListener('beforeselected', (e) => {
    ...
    it('should veto before deselected', async () => {
        let veto;
        pieChart.addEventListener('beforedeselected', (e) => {
            e.detail.response(veto);
        });
        const triggerClick = (el) => el.dispatchEvent(new Event('click', { bubbles: true }));
        pieChart.selectable = true;
...
//# sourceMappingURL=ids-pie-chart-func-test.js.map