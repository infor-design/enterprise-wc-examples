...
describe('Ids Week View e2e Tests', () => {
    ...
    it('should render one day and show correct day', async () => {
        ...
        const hasCssClass = await page.$eval(name, (el) => Array.from(el.shadowRoot.querySelectorAll('.week-view-header-wrapper')).every((item) => item.classList.contains('is-day-view')));
        // Day numeric element content
        const dayNumeric = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-header-day-of-week.is-emphasis')?.textContent);
        // Weekday element content
        const weekDay = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-header-day-of-week:not(.is-emphasis)')?.textContent);
    ...
    it('should change dates', async () => {
        // Previous date
        await page.$eval(name, (el) => el.changeDate('previous'));
        let dayNumeric = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-header-day-of-week.is-emphasis')?.textContent);
        let weekDay = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-header-day-of-week:not(.is-emphasis)')?.textContent);
        expect(dayNumeric).toEqual('7');
        expect(weekDay).toEqual('Sun');
        // Next date
        await page.$eval(name, (el) => el.changeDate('next'));
        await page.$eval(name, (el) => el.changeDate('next'));
        dayNumeric = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-header-day-of-week.is-emphasis')?.textContent);
        weekDay = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-header-day-of-week:not(.is-emphasis)')?.textContent);
        ...
        // Click today
        await page.$eval(name, (el) => el.changeDate('today'));
        dayNumeric = await page.$eval(name, (el) => el.shadowRoot.querySelector('.week-view-header-wrapper.is-today .is-emphasis').textContent);
        expect(+dayNumeric).toEqual(new Date().getDate());
        // Set different years
        await page.evaluate((el) => {
            const element = document.querySelector(el);
    ...
    it('should support changing locale', async () => {
        ...
        await page.waitForFunction(() => !document.querySelector('ids-week-view').shadowRoot.querySelector('.week-view-header-day-of-week:nth-child(1)').classList.contains('is-emphasis'));
        const weekDayHasIsEmphasis = await page.$eval(name, (el) => el.shadowRoot.querySelector('.week-view-header-day-of-week:nth-child(1)').classList.contains('is-emphasis'));
        const dayNumericHasIsEmphasis = await page.$eval(name, (el) => el.shadowRoot.querySelector('.week-view-header-day-of-week:nth-child(2)').classList.contains('is-emphasis'));
        const dayNumeric = await page.$eval(name, (el) => el.shadowRoot.querySelector('.week-view-header-day-of-week.is-emphasis').textContent);
        const weekDay = await page.$eval(name, (el) => el.shadowRoot.querySelector('.week-view-header-day-of-week:not(.is-emphasis)').textContent);
    ...
    it('should hide timeline if current time is out of start/end hour range', async () => {
        ...
        const timelineShiftCssVar = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-hour-row')?.style.cssText);
    ...
    it('should show/hide timeline', async () => {
        ...
        let timeline = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-time-marker'));
        ...
        timeline = await page.$eval(name, (el) => el.shadowRoot?.querySelector('.week-view-time-marker'));
    ...
    it('should change timeline position with interval', async () => {
        ...
        const positionBefore = await page.$eval(name, (el) => el.shadowRoot.querySelector('.week-view-hour-row')?.dataset.diffInMilliseconds);
        await page.waitForTimeout(1000);
        const positionAfter = await page.$eval(name, (el) => el.shadowRoot.querySelector('.week-view-hour-row')?.dataset.diffInMilliseconds);
        expect(positionBefore).not.toEqual(positionAfter);
    });
});
//# sourceMappingURL=ids-week-view-e2e-test.js.map