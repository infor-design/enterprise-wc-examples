...
export default class IdsDataGridFilters {
    ...
    setFilterConditions(conditions) {
        ...
        conditions.forEach((c, i) => {
            ...
            if (!el || hasClass(el, 'disabled') || hasClass(el, 'readonly'))
                return;
        ...
        const checkRow = (row, index) => {
            ...
            for (let i = 0, l = conditions.length; i < l; i++) {
                ...
                if (typeof c.columnData.filterType !== 'function'
                    && typeof c.columnData.filterFunction !== 'function')
                    continue;
                // Use custom filter
                ...
                if (hasClass(filterWrapper, 'disabled') || hasClass(filterWrapper, 'readonly'))
                    continue;
    ...
    attachFilterEventHandlers() {
        ...
        this.root.onEvent(`selected.${this.#id()}`, this.root.wrapper, (e) => {
            ...
            if (!elem || (elem && !(/ids-menu-item/gi.test(elem.nodeName))))
                return;
    ...
    #operators(type, column) {
        ...
        const contentsOperators = () => {
            const val = this.root.datasource.allData.map((row) => escapeHTML(row[column.id]));
...
//# sourceMappingURL=ids-data-grid-filters.js.map