 ...
 */
let IdsListView = class IdsListView extends Base {
    constructor() {
        super();
    }
  ...
  `;
     ...
     */
    get ds() { return this.datasource?.allData || this.data; }
    /**
     ...
     * @param {Array | null} value The array to use
     */
    set loaded(value) { this.setAttribute(attributes.LOADED, String(!!value)); }
    get loaded() { return stringToBool(attributes.LOADED); }
    /**
     * Set the data array of the listview
     * @param {Array | null} value The array to use
     */
    ...
    get dataKeys() { return this.datasource.dataKeys || []; }
    /**
    ...
    get selected() {
        ...
        return this.selectable === 'single'
            ? singleArgs(this.ds.findIndex((d) => d.itemSelected))
            : this.ds.map((data, index) => ({ index, data })).filter((d) => d.data.itemSelected);
    }
    /**
