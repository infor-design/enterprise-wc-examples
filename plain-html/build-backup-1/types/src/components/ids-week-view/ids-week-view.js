...
import { daysDiff, addDate, subtractDate, firstDayOfWeekDate, isTodaysDate, isValidDate, lastDayOfWeekDate } from '../../utils/ids-date-utils/ids-date-utils';
...
import { clearAnimationInterval, requestAnimationInterval } from '../../utils/ids-timer-utils/ids-timer-utils';
/**
 ...
 */
let IdsWeekView = class IdsWeekView extends Base {
    dayMap = [];
    ...
    #renderTimeline() {
        ...
        this.container?.querySelectorAll('.week-view-hour-row:nth-child(1) td')
            .forEach((item) => item.insertAdjacentHTML('afterbegin', '<div class="week-view-time-marker"></div>'));
        const hoursDiff = this.endHour - this.startHour + 1;
    ...
    #clearInterval() {
        if (this.#timelinePositionInterval)
            clearAnimationInterval(this.#timelinePositionInterval);
    }
    /**
...
IdsWeekView = __decorate([
    customElement('ids-week-view'),
    scss(styles),
    __metadata("design:paramtypes", [])
], IdsWeekView);
export default IdsWeekView;
//# sourceMappingURL=ids-week-view.js.map