...
// Import Utils
import { addDate, daysDiff, daysInMonth, firstDayOfMonthDate, firstDayOfWeekDate, gregorianToUmalqura, isValidDate, lastDayOfMonthDate, subtractDate, umalquraToGregorian, weeksInMonth, weeksInRange, removeDateRange } from '../../utils/ids-date-utils/ids-date-utils';
...
import IdsCalendarEvent from '../ids-calendar/ids-calendar-event';
import { getDateValuesFromString } from '../ids-date-picker/ids-date-picker-common';
import { BASE_Y_OFFSET, MIN_MONTH, MAX_MONTH, MAX_EVENT_COUNT, WEEK_LENGTH } from './ids-month-view-common';
/**
 ...
 */
let IdsMonthView = class IdsMonthView extends Base {
    constructor() {
        super();
    }
    connectedCallback() {
    ...
    changeDate(type, limitMonth = false) {
        ...
        if (type === 'next-year') {
            if (this.locale?.isIslamic()) {
                const umalqura = gregorianToUmalqura(this.activeDate);
                const gregorian = umalquraToGregorian(umalqura.year + 1, umalqura.month, umalqura.day === 30 ? 1 : umalqura.day);
                this.day = gregorian.getDate();
                this.month = gregorian.getMonth();
                this.year = gregorian.getFullYear();
            }
    ...
    #setRangeSelection(year, month, day) {
        ...
        if (rangeStarted && !maxRangeExceeded && (canSelectBoth || selectBackward || selectForward)) {
            if (minRangeExceeded) {
                ...
                this.rangeSettings.start = subtractDate(this.rangeSettings.end, minDays, 'days');
    ...
    #clearRangeClasses() {
        this.container?.querySelectorAll('td')
            .forEach((item) => item.classList.remove('range-next', 'range-prev', 'range-selection', 'not-included'));
    }
    /**
    ...
    #rangeSelectWeek(year, month, day) {
        const firstDayOfWeek = firstDayOfWeekDate(new Date(year, month, day), this.firstDayOfWeek);
    ...
    isDisabledByDate(date) {
        const { years, dayOfWeek, dates, minDate, maxDate, isEnable } = this.disableSettings;
        const isOutOfDisplayRange = this.#isDisplayRange()
            && (date < this.startDate || date > this.endDate);
        const ifYear = years.some((item) => item === date.getFullYear());
        const ifDayOfWeek = dayOfWeek.some((item) => item === date.getDay());
        const ifDates = dates.some((item) => (new Date(item)).getTime() === date.getTime());
    ...
    #getCellTemplate(weekIndex) {
        ...
        return Array.from({ length: WEEK_LENGTH }).map((_, index) => {
            ...
            const classAttr = buildClassAttrib(isAlternate && 'alternate', legend && 'has-legend', isDisabled && 'is-disabled', (isSelected || isSelectedWithRange) && 'is-selected', monthFormat && 'month-label', isRangeSelection && 'range-selection', isToday && 'is-today');
...
IdsMonthView = __decorate([
    customElement('ids-month-view'),
    scss(styles),
    __metadata("design:paramtypes", [])
], IdsMonthView);
export default IdsMonthView;
//# sourceMappingURL=ids-month-view.js.map