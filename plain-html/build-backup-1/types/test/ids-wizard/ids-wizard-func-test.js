...
describe('IdsWizard Tests', () => {
    let elem;
    beforeEach(async () => {
        elem = createFromTemplate(elem, `<ids-wizard step-number="1">
    ...
    it('render via document.createElement (append laate)', () => {
        const errors = jest.spyOn(global.console, 'error');
        const wizardElem = document.createElement('ids-wizard');
    ...
    it('initializes without step number and it is set to -1', () => {
        elem = createFromTemplate(elem, `<ids-wizard>
        <ids-wizard-step>Step One</ids-wizard-step>
        <ids-wizard-step>Step Two</ids-wizard-step>
        <ids-wizard-step>Step Three</ids-wizard-step>
      </ids-wizard>`);
...
//# sourceMappingURL=ids-wizard-func-test.js.map