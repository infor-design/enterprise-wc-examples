...
describe('Ids Date Picker e2e Tests', () => {
    ...
    it.skip('should handle calendar popup events', async () => {
        // Closed before
        let isOpen = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.visible);
        expect(isOpen).toBeFalsy();
        // Open popup
        await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-trigger-button')?.click());
        isOpen = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.visible);
        expect(isOpen).toBeTruthy();
        // Click to itself
        await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.click());
        isOpen = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.visible);
        expect(isOpen).toBeTruthy();
        // Click outside
        await page.evaluate(() => {
    ...
    it.skip('should set correct date to the calendar popup', async () => {
        // Open popup
        await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-trigger-button')?.click());
        let year = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.year);
        let month = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.month);
        let day = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.day);
        ...
        // Open popup
        await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-trigger-button')?.click());
        year = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.year);
        month = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.month);
        day = await page.$eval('#e2e-datepicker-value', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.day);
        ...
        // Open popup
        await page.$eval('#e2e-datepicker-required', (el) => el.shadowRoot.querySelector('ids-trigger-button')?.click());
        year = await page.$eval('#e2e-datepicker-required', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.year);
        month = await page.$eval('#e2e-datepicker-required', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.month);
        day = await page.$eval('#e2e-datepicker-required', (el) => el.shadowRoot.querySelector('ids-date-picker-popup')?.day);
        expect(year).toEqual(now.getFullYear());
        expect(month).toEqual(now.getMonth());
        expect(day).toEqual(now.getDate());
    });
    it('calendar popup should set correct input date value', async () => {
        ...
        const value = await page.$eval('#e2e-datepicker-value', (el) => el?.value);
    ...
    it.skip('should change when used in calendar toolbar', async () => {
      ...
      `);
        ...
        const hasCssClass = await page.$eval('#e2e-datepicker-toolbar', (el) => el.container.classList.contains('is-calendar-toolbar'));
        const hasTabindex = await page.$eval('#e2e-datepicker-toolbar', (el) => el.container.getAttribute('tabindex') === '0');
    ...
    it('should handle validation', async () => {
        let isRequired = await page.$eval('#e2e-datepicker-required', (el) => el.validate?.includes('required'));
        ...
        isRequired = await page.$eval('#e2e-datepicker-required', (el) => el.validate?.includes('required'));
    ...
    it('should handle locale change', async () => {
        ...
        await page.waitForFunction(() => document.querySelector('ids-container')?.locale?.calendar().name === 'islamic-umalqura');
        ...
        // Wait till calendars load
        await page.waitForFunction(() => document.querySelector('ids-container')?.locale?.calendar().name === 'gregorian');
    ...
    it.skip('should handle month year picker events', async () => {
        await page.evaluate(() => {
            document.querySelector('ids-container').insertAdjacentHTML('afterbegin', '<ids-date-picker id="e2e-monthyear-picker" is-dropdown="true" value="January 2021" month="0" year="2021"></ids-date-picker>');
        });
        ...
        expect(monthSelectedText).toEqual('January');
        expect(yearSelectedText).toEqual('2013');
        await page.$eval('#e2e-monthyear-picker', (el) => el.shadowRoot.querySelector('.picklist-item.is-btn-up.is-year-nav')?.click());
        yearSelectedText = await page.$eval('#e2e-monthyear-picker', (el) => el.shadowRoot.querySelector('.picklist-item.is-year.is-selected')?.textContent);
        expect(yearSelectedText).toEqual('2007');
        await page.$eval('#e2e-monthyear-picker', (el) => el.shadowRoot.querySelector('.picklist-item.is-btn-down.is-year-nav')?.click());
        yearSelectedText = await page.$eval('#e2e-monthyear-picker', (el) => el.shadowRoot.querySelector('.picklist-item.is-year.is-selected')?.textContent);
        expect(yearSelectedText).toEqual('2013');
        ...
        await page.$eval('#e2e-datepicker-legend', (el) => el?.container.querySelector('.popup-btn-apply')?.click());
        const appliedToMonthView = await page.$eval('#e2e-datepicker-legend', (el) => {
        ...
        await page.$eval('#e2e-datepicker-legend', (el) => el?.container.querySelector('.popup-btn-apply')?.click());
        const datePickerValue = await page.$eval('#e2e-datepicker-legend', (el) => el?.value);
        expect(datePickerValue).toEqual('1/15/2022');
    });
    it.skip('should handle range selection', async () => {
        // Settings range to value
        await page.evaluate(() => {
            document.querySelector('ids-container').insertAdjacentHTML('afterbegin', '<ids-date-picker id="e2e-range-picker" use-range="true" value="2/7/2018 - 2/22/2018"></ids-date-picker>');
        });
    ...
    it('should change value on input value change', async () => {
        ...
        await page.$eval('#e2e-datepicker-required', (el) => el?.container.querySelector('ids-trigger-field')?.setAttribute('value', '2021-04-09'));
        let value = await page.$eval('#e2e-datepicker-required', (el) => el?.value);
        expect(value).toEqual('2021-04-09');
        // Reset value in the input
        await page.$eval('#e2e-datepicker-required', (el) => el?.container.querySelector('ids-trigger-field')?.setAttribute('value', ''));
        value = await page.$eval('#e2e-datepicker-required', (el) => el?.value);
        expect(value).toEqual('');
    });
    it.skip('should handle time', async () => {
        await page.evaluate(() => {
            document.querySelector('ids-container').insertAdjacentHTML('afterbegin', '<ids-date-picker id="e2e-datepicker-time" value="2/3/2010 08:24 AM" format="M/d/yyyy hh:mm a"></ids-date-picker>');
            const component = document.querySelector('#e2e-datepicker-time');
            if (component) {
        ...
        let timePicker = await page.$eval('#e2e-datepicker-time', (el) => el?.container.querySelector('ids-time-picker'));
        expect(timePicker).not.toBeNull();
        const getTimeValues = async () => {
            const hours = await page.$eval('#e2e-datepicker-time', (el) => el?.container.querySelector('ids-time-picker')?.container.querySelector('#hours')?.value);
            const minutes = await page.$eval('#e2e-datepicker-time', (el) => el?.container.querySelector('ids-time-picker')?.container.querySelector('#minutes')?.value);
            const seconds = await page.$eval('#e2e-datepicker-time', (el) => el?.container.querySelector('ids-time-picker')?.container.querySelector('#seconds')?.value);
            const period = await page.$eval('#e2e-datepicker-time', (el) => el?.container.querySelector('ids-time-picker')?.container.querySelector('#period')?.value);
            return {
        ...
        await page.evaluate(() => {
            document.querySelector('#e2e-datepicker-time').close();
            document.querySelector('#e2e-datepicker-time').value = '2/3/2010 11:00 PM';
            document.querySelector('#e2e-datepicker-time').open();
        });
        ...
        const value = await page.$eval('#e2e-datepicker-time', (el) => {
        ...
        // Remove time picker
        await page.evaluate(() => {
            document.querySelector('#e2e-datepicker-time').format = 'M/d/yyyy';
        });
        timePicker = await page.$eval('#e2e-datepicker-time', (el) => el?.container.querySelector('ids-time-picker'));
    ...
    it.skip('should handle UTC date format', async () => {
        ...
        // Parsing
        const isValidCalendar = await page.$eval('#e2e-datepicker-value', (el) => {
            const monthView = el?.container.querySelector('ids-month-view');
    ...
    it.skip('should handle different locale and language', async () => {
        ...
        const picklistMonth = await page.$eval('#e2e-datepicker-value', (el) => el?.container.querySelector('ids-month-view')?.container.querySelector('ids-date-picker')?.value);
        const weekDays = await page.$eval('#e2e-datepicker-value', (el) => Array.from(el?.container.querySelector('ids-month-view')?.container.querySelectorAll('.weekday-text')).map((item) => item?.textContent));
...
//# sourceMappingURL=ids-date-picker-e2e-test.js.map