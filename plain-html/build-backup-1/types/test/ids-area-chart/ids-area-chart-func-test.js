...
describe('IdsAreaChart Component', () => {
    ...
    it('should select/deselect by click', () => {
        ...
        const triggerClick = (el) => el.dispatchEvent(new Event('click', { bubbles: true }));
        areaChart.selectable = true;
        let selected = areaChart.selectionElements.filter((el) => el.hasAttribute('selected'));
        let selectedClass = areaChart.selectionElements.filter((el) => el.classList.contains('selected'));
        expect(selected.length).toEqual(0);
        expect(selectedClass.length).toEqual(0);
        let elem = areaChart.shadowRoot.querySelector('.chart-legend-item:nth-child(1)');
        triggerClick(elem);
        selected = areaChart.selectionElements.filter((el) => el.hasAttribute('selected'));
        selectedClass = areaChart.selectionElements.filter((el) => el.classList.contains('selected'));
    ...
    it('should set pre selected group elements', () => {
        ...
        document.body.appendChild(areaChart);
        areaChart.data = ds;
        const selected = areaChart.selectionElements.filter((el) => el.hasAttribute('selected'));
        const selectedClass = areaChart.selectionElements.filter((el) => el.classList.contains('selected'));
        expect(selected.length).toEqual(1);
        expect(selectedClass.length).toEqual(8);
    });
    it('should set pre selected group elements', () => {
    ...
    it('should veto before selected', async () => {
        let veto;
        areaChart.addEventListener('beforeselected', (e) => {
    ...
    it('should veto before deselected', async () => {
        let veto;
        areaChart.addEventListener('beforedeselected', (e) => {
            e.detail.response(veto);
        });
        const triggerClick = (el) => el.dispatchEvent(new Event('click', { bubbles: true }));
        areaChart.selectable = true;
...
//# sourceMappingURL=ids-area-chart-func-test.js.map