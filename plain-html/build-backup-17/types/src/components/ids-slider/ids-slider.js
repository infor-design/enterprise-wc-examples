 ...
 */
let IdsSlider = class IdsSlider extends Base {
    ...
    #setVertical() {
        if (this.vertical) {
            ...
            if (this.type === 'range')
                this.thumbDraggableSecondary?.setAttribute(htmlAttributes.ARIA_ORIENTATION, 'vertical');
            ...
            if (this.type === 'range')
                this.thumbDraggableSecondary?.setAttribute(htmlAttributes.ARIA_ORIENTATION, 'horizontal');
        }
    }
    /**
     * Keep track of RTL
    ...
    #updateProgressBar() {
        if (this.type !== 'range') {
            ...
            if (this.#trackBounds) {
                const { TOP, BOTTOM, LEFT, RIGHT } = this.#trackBounds;
        ...
        if (labelsLength === stepNumber) {
            ...
            if (labels && labelsLength === labelElementsLength) {
                labelElements?.forEach((x, i) => {
                    ...
                    if (this.vertical)
                        x?.classList.add('vertical'); // add vertical styles
                });
            }
        }
        else {
     ...
     */
    get stepNumber() { return parseInt(this.getAttribute(attributes.STEP_NUMBER) ?? '') || 2; }
    #setStepNumber() {
        if (this.type === 'step') {
     ...
     */
    get type() {
        return (this.getAttribute(attributes.TYPE) || DEFAULT_TYPE);
    }
    /**
...
IdsSlider = __decorate([
    customElement('ids-slider'),
    scss(styles),
    __metadata("design:paramtypes", [])
], IdsSlider);
export default IdsSlider;
//# sourceMappingURL=ids-slider.js.map