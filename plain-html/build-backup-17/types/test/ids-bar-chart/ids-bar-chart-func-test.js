...
describe('IdsBarChart Component', () => {
    ...
    it('shows a tooltip on hover', async () => {
        ...
        const tooltip = barChart.container.querySelector('ids-tooltip');
        await processAnimFrame();
    ...
    it('should select/deselect by click', () => {
        ...
        barChart.selectable = true;
        let selected = barChart.selectionElements.filter((el) => el.hasAttribute('selected'));
        expect(selected.length).toEqual(0);
        let elem = barChart.shadowRoot.querySelector('.chart-legend-item:nth-child(1)');
        triggerClick(elem);
        selected = barChart.selectionElements.filter((el) => el.hasAttribute('selected'));
    ...
    it('should set pre selected group elements', async () => {
        ...
        barChart.data = ds;
        await processAnimFrame();
        const selected = barChart.selectionElements.filter((el) => el.hasAttribute('selected'));
        expect(selected.length).toEqual(6);
    });
    ...
    it('should veto before selected', async () => {
        let veto;
        barChart.addEventListener('beforeselected', (e) => {
    ...
    it('should veto before deselected', async () => {
        let veto;
        barChart.addEventListener('beforedeselected', (e) => {
            e.detail.response(veto);
        });
        const triggerClick = (el) => el.dispatchEvent(new Event('click', { bubbles: true }));
        barChart.selectable = true;
    ...
    it('should get/set selected by api', async () => {
        ...
        expect(barChart.getSelected().indexes).toEqual(expect.arrayContaining([
...
//# sourceMappingURL=ids-bar-chart-func-test.js.map