...
const getPercentageTextHtml = ({ progress, type = 'circular' }) => (`<div class="progress-percentage ${type}" part="percentage-text">
...
const getInnerIndicatorHtml = ({ progress, type, percentageVisible, inline }) => {
    ...
    switch (type) {
        ...
        case attributes.LINEAR: {
            ...
            const classStr = buildClassAttrib('linear-indicator', type === 'sticky' && 'sticky', `${!isDeterminate ? 'in' : ''}determinate`);
            return (`<svg
      ...
      ${!(percentageVisible && type !== 'sticky') ? ''
        ...
        // circular
        default: {
            const classStr = buildClassAttrib('circular-indicator', `${!isDeterminate ? 'in' : ''}determinate`, inline && 'inline');
            return (`<svg
        viewbox="0 0 100 100"
        xmlns="http://www.w3.org/2000/svg"
        ${classStr}
...
//# sourceMappingURL=ids-loading-indicator-attributes.js.map