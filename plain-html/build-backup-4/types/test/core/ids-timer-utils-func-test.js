 ...
 */
import { clearAnimationInterval, clearAnimationTimeout, cssTransitionTimeout, requestAnimationInterval, requestAnimationTimeout, } from '../../src/utils/ids-timer-utils/ids-timer-utils';
...
describe('IdsTimerUtils tests (requestAnimationFrame)', () => {
    ...
    it('can execute functions on a timeout', async () => {
        const timeout = await requestAnimationTimeout(() => el?.classList.add('complete'), 100);
    ...
    it('can cancel a timeout function and prevent it from occuring', async () => {
        const timeout = requestAnimationTimeout(() => el?.classList.add('complete'), 100);
        expect(timeout.value).toBeDefined();
        await wait(50);
    ...
    it('can execute functions on an interval', async () => {
        let count = 0;
        const interval = await requestAnimationInterval(() => {
            count += 1;
        }, 50);
...
//# sourceMappingURL=ids-timer-utils-func-test.js.map