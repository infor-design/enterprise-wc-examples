...
import { VIEWS, PARAGRAPH_SEPARATORS, BLOCK_ELEMENTS, FONT_SIZES, CLASSES, EDITOR_DEFAULTS, EDITOR_ATTRIBUTES, qs, qsAll, rgbToHex } from './ids-editor-shared';
import { cleanHtml, trimContent } from './ids-editor-clean-utils';
import { blockElem, selectionBlockElems, saveSelection, restoreSelection, selectionParents, findElementInSelection } from './ids-editor-selection-utils';
import { parseTemplate, editorTemplate, btnEditorModeTemplate, btnSourceModeTemplate, toolbarTemplate, errorMessageTemplate, hyperlinkModalTemplate, insertimageModalTemplate } from './ids-editor-templates';
import { handlePasteAsPlainText, handlePasteAsHtml } from './ids-editor-handle-paste';
 ...
 */
let IdsEditor = class IdsEditor extends Base {
    constructor() {
        super();
    }
    validationElems;
    ...
    #initModals() {
        ...
        if (hyperlinkBtn) {
            ...
            const { url, classes, targets, targetSelected, isClickable, showIsClickable } = this.#modals.defaults.hyperlink;
    ...
    #onSelectedToolbar(e) {
        ...
        let target = null;
        if (/^ids-menu-item$/i.test(elem.nodeName))
            target = elem.menu?.target;
        if (/^ids-button$/i.test(elem.nodeName))
            target = elem;
        ...
        if (this.#actions[action]) {
            ...
            if (action === 'formatblock') {
                ...
                if (menuBtn)
                    menuBtn.text = elem.text || elem.textContent?.trim();
        ...
        if (key === 'hyperlink') {
            ...
            this.onEvent(`change.editor-modal-${key}-checkbox-remove`, removeElem, (e) => {
                elemsToDisable.forEach((elem) => {
                    if (elem)
                        elem.disabled = e.detail.checked;
                });
            });
        }
    }
    ...
    #attachKeyboardEvents() {
        ...
        this.onEvent('keydown.editor-container', this.container, (e) => {
            ...
            if (keys.indexOf(key) > -1 && (e.ctrlKey || e.metaKey)) {
                const action = mapped[key]?.filter((a) => (e.shiftKey === hasKey(a, 'shift') && e.altKey === hasKey(a, 'alt'))).flat();
                if (action?.length) {
     ...
     * @returns {void}
     */
    setLabelText(value = this.state?.label) {
        ...
        if (labelEl)
            labelEl.innerHTML = shouldDisplayLabel ? value : '';
        if (sourceLabel)
            sourceLabel.innerHTML = shouldDisplayLabel ? this.sourceTextareaLabel() : '';
    }
    /**
...
IdsEditor = __decorate([
    customElement('ids-editor'),
    scss(styles),
    __metadata("design:paramtypes", [])
], IdsEditor);
export default IdsEditor;
//# sourceMappingURL=ids-editor.js.map