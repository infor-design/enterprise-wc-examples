...
const DEFAULT_SPINBOX_HTML = (`<ids-spinbox
    value="0"
    min="-25"
    max="25"
    step="5"
...
describe('IdsSpinbox Component', () => {
    ...
    it('presses the increment and decrement buttons with no errors', async () => {
        ...
        await processAnimFrame();
        expect(document.querySelectorAll('ids-spinbox').length).toEqual(1);
        const [decrementButton, incrementButton] = [...elem.querySelectorAll('ids-trigger-button')];
    ...
    it('increments the value until max, and then cannot increment anymore', async () => {
        ...
        decrementButton, incrementButton] = [...elem.querySelectorAll('ids-trigger-button')];
        do {
    ...
    it('disables its trigger buttons if the field is disabled or readonly', async () => {
        elem = await createFromTemplate(elem, `<ids-spinbox readonly value="10"></ids-spinbox>`);
        await processAnimFrame();
        const getIdsButtons = () => ([...elem.querySelectorAll('ids-trigger-button')]);
        await waitForTimeout(() => expect(getIdsButtons().find((el) => el.hasAttribute('disabled'))).not.toEqual(undefined));
...
//# sourceMappingURL=ids-spinbox-func-test.js.map