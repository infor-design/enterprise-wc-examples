 ...
 */
let IdsUploadAdvancedFile = class IdsUploadAdvancedFile extends Base {
    constructor() {
        super();
    }
    /**
    ...
    setStatus() {
        ...
        if (shouldTrigger && this.status !== IdsUploadAdvancedShared.STATUS.inProcess) {
            ...
            if (this.status)
                this.dispatchChangeEvent(events[camelCase(this.status)]);
        }
    }
    /**
     * Handle events
    ...
    set status(value) {
        if (this.status === value)
            return;
        if (Object.values(IdsUploadAdvancedShared.STATUS).indexOf(value) > -1) {
