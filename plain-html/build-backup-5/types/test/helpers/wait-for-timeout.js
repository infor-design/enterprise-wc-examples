 ...
 */
export default async function waitForTimeout(selectorOrAssertionCb, { container = window.document, timeout = 3000, interval = 40, onTimeout, mutationObserverOptions = {
    subtree: true,
    childList: true,
    attributes: true,
    characterData: true,
    ...
    switch (typeof selectorOrAssertionCb) {
        ...
        case 'string':
            return waitForTimeout(() => {
                ...
                const elem = (container.querySelector?.(selectorOrAssertionCb)
    ...
    return new Promise(async (resolve, reject) => {
        ...
        function checkRealTimersCallback() {
            if (areJestFakeTimersEnabled()) {
                const error = new Error('Changed from using real timers to fake timers while using waitFor. '
        ...
        if (usingJestFakeTimers) {
            ...
            while (!finished) {
                if (!areJestFakeTimersEnabled()) {
                    const error = new Error('Changed from using fake timers to real timers while using waitFor. '
                        + 'This is not allowed and will result in very strange behavior. '
