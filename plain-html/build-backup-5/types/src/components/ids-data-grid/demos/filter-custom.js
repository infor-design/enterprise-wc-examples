...
const myCustomFilter = (opt) => {
    ...
    let isMatch = false;
    if (Number.isNaN(val.condition) || Number.isNaN(val.data))
        return isMatch;
    if (operator === 'equals')
        isMatch = (val.data === val.condition);
    if (operator === 'greater-than')
        isMatch = (val.data > val.condition);
    if (operator === 'greater-equals')
        isMatch = (val.data >= val.condition);
    if (operator === 'less-than')
        isMatch = (val.data < val.condition);
    if (operator === 'less-equals')
        isMatch = (val.data <= val.condition);
    if (operator === 'start-with')
        isMatch = (val.data.toString().startsWith(val.condition.toString()));
    return isMatch;
};
(async function init() {
    const columns = [];
...
const colorInput = document.body.querySelector('#filter-input');
/**
...
//# sourceMappingURL=filter-custom.js.map